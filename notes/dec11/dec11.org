...
* The Rational Speech Act framework
  ...
  an more probability mass to the utterance that maximizes the utility in
...
* Doing it all in Haskell
  ...
  exponentiates the second constant to the first.
  #+begin_src haskell
    data Constant (φ :: Type) where
      ...
      ...
      ...
  #+end_src
  Finally the constants representing possible world and utterance priors receive
  the types of probabilistic programs.
  #+begin_src haskell
    data Constant (φ :: Type) where
      ...
  ...
  That's it! If we want to test this out, we could, say, check the mass that ~l 1~
  assigns to some example possible world ~i0~, when it is applied to the utterance
  ~someoneSleeps~.
  #+begin_src haskell
